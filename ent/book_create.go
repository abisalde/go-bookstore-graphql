...
func (bc *BookCreate) SetPublication(s string) *BookCreate {
	bc.mutation.SetPublication(s)
	return bc
}
...
func (bc *BookCreate) check() error {
	...
	if _, ok := bc.mutation.Publication(); !ok {
		return &ValidationError{Name: "publication", err: errors.New(`ent: missing required field "Book.publication"`)}
	}
	return nil
}
...
func (bc *BookCreate) createSpec() (*Book, *sqlgraph.CreateSpec) {
	...
	if value, ok := bc.mutation.Publication(); ok {
		_spec.SetField(book.FieldPublication, field.TypeString, value)
		_node.Publication = value
	}
	return _node, _spec
}
