...
// SetPublication sets the "publication" field.
func (m *BookMutation) SetPublication(s string) {
	m.publication = &s
}

// Publication returns the value of the "publication" field in the mutation.
func (m *BookMutation) Publication() (r string, exists bool) {
	v := m.publication
	if v == nil {
		return
	}
...
// OldPublication returns the old "publication" field's value of the Book entity.
// If the Book object wasn't provided to the builder, the object is fetched from the database.
// An error is returned if the mutation operation is not UpdateOne, or the database query fails.
func (m *BookMutation) OldPublication(ctx context.Context) (v string, err error) {
	if !m.op.Is(OpUpdateOne) {
		return v, errors.New("OldPublication is only allowed on UpdateOne operations")
	}
	if m.id == nil || m.oldValue == nil {
		return v, errors.New("OldPublication requires an ID field in the mutation")
	}
	oldValue, err := m.oldValue(ctx)
	if err != nil {
		return v, fmt.Errorf("querying old value for OldPublication: %w", err)
	}
	return oldValue.Publication, nil
}

// ResetPublication resets all changes to the "publication" field.
func (m *BookMutation) ResetPublication() {
	m.publication = nil
}
...
func (m *BookMutation) Fields() []string {
	...
	if m.publication != nil {
		fields = append(fields, book.FieldPublication)
	}
	return fields
}
